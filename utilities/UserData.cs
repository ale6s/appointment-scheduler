...
namespace Scheduling.utilities
{
    internal class UserData
    {
        ...
        public List<string> getUserStartDate(string customerId, string currentApt)
        {
            List<string> list = new List<string>();

            string query = "SELECT start FROM appointment WHERE customerId='" + customerId + "'AND appointmentId !='" + currentApt + "'";
            try
            {
                MySqlCommand cmd = new MySqlCommand(query, GlobalMembers.DBConn);
                MySqlDataReader reader = cmd.ExecuteReader();
                ...
                while (reader.Read())
                {
                    string dbStart = Convert.ToDateTime(reader["start"]).ToLocalTime().ToString();
        ...
        public List<string> getUserEndDate(string customerId, string currentApt)
        {
            List<string> list = new List<string>();

            string query = "SELECT end FROM appointment WHERE customerId='" + customerId + "'AND appointmentId !='" + currentApt + "'";
            try
            {
                MySqlCommand cmd = new MySqlCommand(query, GlobalMembers.DBConn);
                MySqlDataReader reader = cmd.ExecuteReader();
                ...
                while (reader.Read())
                {
                    string dbEnd = Convert.ToDateTime(reader["end"]).ToLocalTime().ToString();
        ...
        public Boolean getAptConnectedToCustomer(int customerId)
        {
            string query = "SELECT Count(customerId) FROM appointment WHERE customerId='" + customerId + "'";
            ...
            if (reader.Read())
            {
                int dbCount = Convert.ToInt32(reader["Count(customerId)"]);
